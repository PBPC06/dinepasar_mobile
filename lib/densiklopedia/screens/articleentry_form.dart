...
class ArticleEntryForm extends StatefulWidget {
  ...
  const ArticleEntryForm({Key? key, this.article}) : super(key: key); // Menambahkan 'article' dalam konstruktor
              ...
              children: [
                ...
                Center(
                  child: ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        if (widget.article == null) {
                          try {
                            ...
                            if (response['status'] == 'success') {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    "Gagal menambahkan artikel: ${response['message'] ?? 'Unknown error'}"
                                  ),
                                ),
                              );
                            }
                          ...
                          final response = await request.post(
                            "http://namira-aulia31-dinepasar.pbp.cs.ui.ac.iddensiklopedia/edit-flutter/${widget.article!.id}/",
                            {
                              "judul": _judul,
                              "subjudul": _subjudul,
                              "gambar": _gambar,
                    ...
                    child: Text(
                      widget.article == null ? "Simpan" : "Simpan Perubahan",
                      style: const TextStyle(color: Colors.white), // Mengubah warna teks menjadi putih
                    ),
                  ),
                ),
              ],
